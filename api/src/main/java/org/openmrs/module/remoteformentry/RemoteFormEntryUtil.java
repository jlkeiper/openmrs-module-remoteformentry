...
public class RemoteFormEntryUtil {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (StringUtils.isBlank(typeId)) {
				...
				try {
					...
					if (pat == null) {
						throw new RemoteFormEntryException("Could not find relationship type: " + typeId);
					}
					...
					if (relative == null) {
						...
						if (StringUtils.isNotBlank(identifierStr) &&
							...
							Location loc = null;
							try {
								loc = Context.getLocationService().getLocation(Integer.parseInt(locationId));
							} catch (NumberFormatException e) {
								throw new RemoteFormEntryException("Could not get location from value: " + locationId);
							}

							pid = new PatientIdentifier(identifierStr, pit, loc);
						}

						// if the relative has a patient identifier, assume it is a patient, add the identifier, then save it
						if (pid != null) {
	...
	/**
	 * Turns off Form Entry Queue Processor task and creates / verifies the Pre Processor task
	 */
	public static void updateScheduledTasksForRemote() {
		boolean preProcessorStarted = false;

		// find form entry scheduled tasks and stop them
		Context.addProxyPrivilege(PrivilegeConstants.MANAGE_SCHEDULER);

		for (TaskDefinition definition : Context.getSchedulerService().getRegisteredTasks()) {

			// check for a started form entry task
			if (OpenmrsUtil.nullSafeEquals(ProcessFormEntryQueueTask.class.getCanonicalName(), definition.getTaskClass()) && definition.getStarted()) {
				try {
					Context.getSchedulerService().shutdownTask(definition);
				} catch (SchedulerException e) {
					log.warn("Could not turn off Form Entry scheduled task; please turn off manually.");
				}
			}

			// check for pre process form entry task
			else if (OpenmrsUtil.nullSafeEquals(PreProcessFormEntryQueueTask.class.getCanonicalName(), definition.getTaskClass())) {
				if (!preProcessorStarted) {
					if (!definition.getStarted()) {
						try {
							Context.getSchedulerService().scheduleTask(definition);
						} catch (SchedulerException e) {
							log.warn("Could not turn on Pre Process Form Entry scheduled task; please turn on manually.");
						}
					}
					preProcessorStarted = true;
				} else {
					Context.getSchedulerService().deleteTask(definition.getId());
				}
			}
		}
		...
		if (!preProcessorStarted) {
			TaskDefinition pp = new TaskDefinition();
			pp.setName("Pre Process Form Entry Queue");
			pp.setDescription("Processes Form Entry data into Remote Form Entry storage before sending through Form Entry.");
			pp.setTaskClass(PreProcessFormEntryQueueTask.class.getCanonicalName());
			pp.setStartOnStartup(true);
			pp.setStartTime(new Date());
			pp.setRepeatInterval(60l);
			pp.setUuid(RemoteFormEntryConstants.TASK_PRE_PROCESS_FORM_ENTRY_QUEUE);

			Context.getSchedulerService().saveTask(pp);
			Context.flushSession();

			log.info("Created Pre Process Form Entry Queue task, id: " + pp.getId());

			// schedule it
			try {
				Context.getSchedulerService().scheduleTask(pp);
			} catch (SchedulerException e) {
				log.warn("Could not turn on Pre Process Form Entry scheduled task; please turn on manually.");
			}
		}
